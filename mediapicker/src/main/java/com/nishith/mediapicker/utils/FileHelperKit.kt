package com.nishith.mediapicker.utils
...
object FileHelperKit {
    ...
    private fun getVideoFile(context: Context,extension: String): File {

        val baseDir = context.filesDir?.absolutePath

        val storageDir = File(
                baseDir,
                "${context.getString(R.string.app_name)}/${"DOC_" + System.currentTimeMillis() + "_${extension}"}"
        )
    ...
    fun getFileFromUri(uri: Uri?,context: Context) : File?{
        kotlin.runCatching {
            ...
            val copedFile = getVideoFile(context, getExtension(displayName)!!)
    ...
    /**
     *   To save bitmap in file.
     *
     *   @param bitmap bitmap to save.
     *   @param file file in which bitmap should save.
     */

    fun saveImage(bitmap: Bitmap?, file: File?): File? {
        try {
    ...
    fun compressImage(filePath: String?, fileName: String): String? {
        ...
        try {
            ...
            val bitmap = Bitmap.createBitmap(scaledBitmap, 0, 0, scaledBitmap.width, scaledBitmap.height, getMatrix(file), true)
        ...
        try {
            ...
            scaledBitmap!!.compress(Bitmap.CompressFormat.JPEG, 55, out)
