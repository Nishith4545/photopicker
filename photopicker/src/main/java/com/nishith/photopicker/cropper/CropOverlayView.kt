...
import com.nishith.photopicker.cropper.CropImageView.CropShape
import com.nishith.photopicker.cropper.CropImageView.Guidelines
...
@JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    private fun initCropWindow() {
        val leftLimit = max(
            getRectLeft(mBoundsPoints).toDouble(), 0.0
        ).toFloat()
        val topLimit = max(
            getRectTop(mBoundsPoints).toDouble(), 0.0
        ).toFloat()
        val rightLimit = min(
            getRectRight(mBoundsPoints).toDouble(),
            width.toDouble()
        ).toFloat()
        val bottomLimit = min(
            getRectBottom(mBoundsPoints).toDouble(),
            height.toDouble()
        ).toFloat()
        ...
        if (mInitialCropWindowRect.width() > 0 && mInitialCropWindowRect.height() > 0) {
            ...
            if (bitmapAspectRatio > mTargetAspectRatio) {
                ...
                val cropWidth = max(
                    mCropWindowHandler.minCropWidth.toDouble(),
                    (rect.height() * mTargetAspectRatio).toDouble()
                ).toFloat()
                ...
                // Limits the aspect ratio to no less than 40 wide or 40 tall
                val cropHeight = max(
                    mCropWindowHandler.minCropHeight.toDouble(),
                    (rect.width() / mTargetAspectRatio).toDouble()
                ).toFloat()
    ...
    private fun fixCropWindowRectByRules(rect: RectF) {
        ...
        if (mCalcBounds.width() > 0 && mCalcBounds.height() > 0) {
            val leftLimit = max(mCalcBounds.left.toDouble(), 0.0).toFloat()
            val topLimit = max(mCalcBounds.top.toDouble(), 0.0).toFloat()
            val rightLimit = min(mCalcBounds.right.toDouble(), width.toDouble())
                .toFloat()
            val bottomLimit = min(mCalcBounds.bottom.toDouble(), height.toDouble())
                .toFloat()
        ...
        if (isFixAspectRatio && abs((rect.width() - rect.height() * mTargetAspectRatio).toDouble()) > 0.1) {
            if (rect.width() > rect.height() * mTargetAspectRatio) {
                val adj =
                    (abs((rect.height() * mTargetAspectRatio - rect.width()).toDouble()) / 2).toFloat()
                rect.left += adj
                rect.right -= adj
            } else {
                val adj =
                    (abs((rect.width() / mTargetAspectRatio - rect.height()).toDouble()) / 2).toFloat()
                rect.top += adj
                rect.bottom -= adj
            }
        }
    ...
    private fun drawCorners(canvas: Canvas) {
        if (mBorderCornerPaint != null) {
            ...
            val w = (
                    cornerWidth / 2
                            + (if (mCropShape == CropShape.RECTANGLE) mBorderCornerOffset else 0f))
    ...
    private fun onActionDown(x: Float, y: Float) {
        mMoveHandler = mCropShape?.let { mCropWindowHandler.getMoveHandler(x, y, mTouchRadius, it) }
