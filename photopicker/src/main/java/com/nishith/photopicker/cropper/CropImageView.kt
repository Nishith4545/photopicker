...
import com.nishith.photopicker.cropper.BitmapUtils.cropBitmap
import com.nishith.photopicker.cropper.BitmapUtils.cropBitmapObjectHandleOOM
import com.nishith.photopicker.cropper.BitmapUtils.getRectBottom
import com.nishith.photopicker.cropper.BitmapUtils.getRectCenterX
import com.nishith.photopicker.cropper.BitmapUtils.getRectCenterY
import com.nishith.photopicker.cropper.BitmapUtils.getRectFromPoints
import com.nishith.photopicker.cropper.BitmapUtils.getRectHeight
import com.nishith.photopicker.cropper.BitmapUtils.getRectLeft
import com.nishith.photopicker.cropper.BitmapUtils.getRectRight
import com.nishith.photopicker.cropper.BitmapUtils.getRectTop
import com.nishith.photopicker.cropper.BitmapUtils.getRectWidth
import com.nishith.photopicker.cropper.BitmapUtils.resizeBitmap
import com.nishith.photopicker.cropper.BitmapUtils.rotateBitmapByExif
import com.nishith.photopicker.cropper.BitmapUtils.writeTempStateStoreBitmap
import com.nishith.photopicker.cropper.CropOverlayView.CropWindowChangeListener
...
class CropImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
     ...
     * bitmap requires saving it to file which can be expensive. default: false.
     */
    var isSaveBitmapToInstanceState: Boolean = false
    ...
    fun rotateImage(degrees: Int) {
        ...
        if (mBitmap != null) {
            ...
            (mZoom / sqrt(
                (BitmapUtils.POINTS2[4] - BitmapUtils.POINTS2[2]).pow(2.0f) + (BitmapUtils.POINTS2[5] - BitmapUtils.POINTS2[3]).pow(
                    2.0f
                )
            )).toFloat()
        ...
        if (state is Bundle) {
            ...
            if (mBitmapLoadingWorkerTask == null && imageUri == null && mBitmap == null && mImageResource == 0) {
                ...
                if (uri != null) {
                    ...
                    if (key != null) {
                        val stateBitmap =
                            if (BitmapUtils.mStateBitmap != null && BitmapUtils.mStateBitmap!!.first == key
                ...
                mCropOverlayView!!.cropShape = CropShape.valueOf(
    ...
    private fun applyImageMatrix(width: Float, height: Float, center: Boolean, animate: Boolean) {
        if (mBitmap != null && width > 0 && height > 0) {
            ...
            val scale = min(
                (width / getRectWidth(mImagePoints)).toDouble(),
                (height / getRectHeight(mImagePoints)).toDouble()
            ).toFloat()
            ...
            if (center) {
                ...
                mZoomOffsetX =
                    if (width > getRectWidth(mImagePoints)
                    ) 0f
                    else (max(
                        min(
                            (width / 2 - cropRect!!.centerX()).toDouble(),
                            -getRectLeft(mImagePoints).toDouble()
                        ),
                        (getWidth() - getRectRight(mImagePoints)).toDouble()
                    ) / scaleX).toFloat()
                mZoomOffsetY =
                    if (height > getRectHeight(mImagePoints)
                    ) 0f
                    else (max(
                        min(
                            (height / 2 - cropRect!!.centerY()).toDouble(),
                            -getRectTop(mImagePoints).toDouble()
                        ),
                        (getHeight() - getRectBottom(mImagePoints)).toDouble()
                    ) / scaleY).toFloat()
            } else {
                // adjust the zoomed area so the crop window rectangle will be inside the area in case it
                // was moved outside
                mZoomOffsetX = (min(
                    max((mZoomOffsetX * scaleX).toDouble(), -cropRect!!.left.toDouble()),
                    (-cropRect.right + width).toDouble()
                ) / scaleX).toFloat()
                mZoomOffsetY = (min(
                    max((mZoomOffsetY * scaleY).toDouble(), -cropRect.top.toDouble()),
    ...
    private fun updateImageBounds(clear: Boolean) {
        if (mBitmap != null && !clear) {
            ...
            val scaleFactorWidth =
                100f * mLoadedSampleSize / getRectWidth(mScaleImagePoints)
            val scaleFactorHeight =
                100f * mLoadedSampleSize / getRectHeight(mScaleImagePoints)
