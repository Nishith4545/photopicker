...
internal class CropWindowHandler {
    ...
    val minCropWidth: Float
        /** Minimum width in pixels that the crop window can get.  */
        get() = max(
            mMinCropWindowWidth.toDouble(),
            (mMinCropResultWidth / scaleFactorWidth).toDouble()
        ).toFloat()

    val minCropHeight: Float
        /** Minimum height in pixels that the crop window can get.  */
        get() = max(
            mMinCropWindowHeight.toDouble(),
            (mMinCropResultHeight / scaleFactorHeight).toDouble()
        ).toFloat()

    val maxCropWidth: Float
        /** Maximum width in pixels that the crop window can get.  */
        get() = min(
            mMaxCropWindowWidth.toDouble(),
            (mMaxCropResultWidth / scaleFactorWidth).toDouble()
        ).toFloat()

    val maxCropHeight: Float
        /** Maximum height in pixels that the crop window can get.  */
        get() = min(
            mMaxCropWindowHeight.toDouble(),
            (mMaxCropResultHeight / scaleFactorHeight).toDouble()
        ).toFloat()
