...
internal class CropWindowMoveHandler(
    ...
    private val mMinCropWidth = cropWindowHandler.minCropWidth

    /** Minimum width in pixels that the crop window can get.  */
    private val mMinCropHeight = cropWindowHandler.minCropHeight

    /** Maximum height in pixels that the crop window can get.  */
    private val mMaxCropWidth = cropWindowHandler.maxCropWidth

    /** Maximum height in pixels that the crop window can get.  */
    private val mMaxCropHeight = cropWindowHandler.maxCropHeight
        ...
        if (aspectRatio > 0) {
            ...
            if (newHeight < mMinCropHeight) {
                newLeft = max(
                    ...
                    (rect.right - mMinCropHeight * aspectRatio).toDouble()
                )
                    .toFloat()
                newHeight = ((rect.right - newLeft) / aspectRatio).toFloat()
            }
            ...
            if (newHeight > mMaxCropHeight) {
                newLeft = max(
                    bounds.left.toDouble(),
                    (rect.right - mMaxCropHeight * aspectRatio).toDouble()
                )
                    .toFloat()
                newHeight = ((rect.right - newLeft) / aspectRatio).toFloat()
            }
        ...
        if (aspectRatio > 0) {
            ...
            if (newHeight < mMinCropHeight) {
                newRight = min(
                    ...
                    (rect.left + mMinCropHeight * aspectRatio).toDouble()
                )
                    .toFloat()
            ...
            if (newHeight > mMaxCropHeight) {
                newRight = min(
                    bounds.right.toDouble(),
                    (rect.left + mMaxCropHeight * aspectRatio).toDouble()
                )
                    .toFloat()
                newHeight = (newRight - rect.left) / aspectRatio
            }
    ...
    private fun adjustTop(
        ...
        if (aspectRatio > 0) {
            ...
            if (newWidth < mMinCropWidth) {
                newTop = max(
                    bounds.top.toDouble(),
                    (rect.bottom - (mMinCropWidth / aspectRatio)).toDouble()
                ).toFloat()
                newWidth = ((rect.bottom - newTop) * aspectRatio).toFloat()
            }
            ...
            if (newWidth > mMaxCropWidth) {
                newTop = max(
                    bounds.top.toDouble(),
                    (rect.bottom - (mMaxCropWidth / aspectRatio)).toDouble()
                ).toFloat()
                newWidth = ((rect.bottom - newTop) * aspectRatio).toFloat()
            }
        ...
        if (aspectRatio > 0) {
            ...
            if (newWidth < mMinCropWidth) {
                newBottom = min(
                    ...
                    (rect.top + mMinCropWidth / aspectRatio).toDouble()
                )
                    .toFloat()
            ...
            if (newWidth > mMaxCropWidth) {
                newBottom = min(
                    bounds.bottom.toDouble(),
                    (rect.top + mMaxCropWidth / aspectRatio).toDouble()
                )
                    .toFloat()
                newWidth = (newBottom - rect.top) * aspectRatio
            }
